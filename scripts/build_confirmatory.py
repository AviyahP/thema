"""Build and export the ontology the confirmatory calibration licenses.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--directory", default="recurrent_dag_confirmatory")
    ...
    kept: list[tuple[np.ndarray, float, dict[int, float]]] = []
    for seed_grouping, family in families(list(completed), completed, pool):
        ...
        if round(float(support), SUPPORT_DECIMALS) < threshold_for(size):
            continue
        _candidate, inclusion = family_members(family, pool, ready.present, words)
        kept.append((member_bits, float(support), inclusion))

    print(f"  families kept: {len(kept):,} of the pool's completed groupings")
