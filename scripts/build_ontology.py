...
def stamp(scope: str, digest: str, rows: int) -> str:
    ...
    Returns:
        A one-line stamp.
    """
    return f"scope={scope} descriptions={rows} sha256={digest[:16]} model={BIOLORD_MODEL}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    header = stamp(scope, digest, len(keys))
